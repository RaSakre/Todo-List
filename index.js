...
tasksList.addEventListener("click", function (evt) {
  const taskCompleteContent = completedTaskTemplate
    .querySelector(".todo-task")
    .cloneNode(true); // Клик по кнопке галочки перемещает задачу в список выполненных
...
function formAddTaskSubmit(evt) {
  ...
  postRequest(taskInput.value).then((data) => {
    ...
    const taskContent = taskTemplate
      .querySelector(".todo-task")
      .cloneNode(true); // Клик по кнопке добавления задачи добавляет её в список задач
